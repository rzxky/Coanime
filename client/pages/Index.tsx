...
export default function Index() {
  ...
  const { data: byGenre } = useQuery({
    queryKey: ["genre", genre],
    queryFn: () => fetchByGenre(String(genre)),
  });
  const { data: byType } = useQuery({
    queryKey: ["type", type],
    queryFn: () => fetchByType(type),
  });
        ...
        <div className="mb-6 flex items-end justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              Minimal, fast, and beautiful anime streaming experience. Discover
              trending shows, explore seasons, and watch trailers.
            </p>
          </div>
        </div>
            ...
            <CarouselContent>
              {trending.slice(0, 10).map((a) => (
                <CarouselItem
                  key={a.mal_id}
                  className={cn(
                    "basis-3/4 sm:basis-1/2 md:basis-1/3 lg:basis-1/4",
                  )}
                >
                  <a
                    href={`/watch/${a.mal_id}`}
                    className="group block overflow-hidden rounded-lg border bg-card/60 shadow transition-colors"
                  >
